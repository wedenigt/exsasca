...
def psdd_read(filename, manager):
    """Read an PSDD from file"""
    ...
    for line in f:
        ...
        if line.startswith('c'):
            ...
            sdd_id, vtree_id, var, log_prob_true = line[2:].split(' ')
            vtree_node = vtree_nodes[vtree_id]
            PSddNode(node_type=PSddNode.TRUE, alpha=None, vtree=vtree_node, manager=manager)
            node_id = int()
            node = manager.true
...
def sdd_read(filename, manager, show_progress=False):
    """Read an SDD from file"""
    vtree_nodes = manager.vtree.to_list()
    f = open(filename, 'r')
    # add number of lines to tqdm
    iter = f if not show_progress else tqdm(f, desc='Reading SDD')
    for line in iter:
        node = None
        if line.startswith('c'):
            continue
...
def psdd_wedenigt_read(filename, pmanager, device, get_num_nodes=False, tqdm=False):
    """Read a PSDD (Jason) from file (not well tested!!!). Converts theta into torch tensors."""
    var_to_vtree = pmanager.vtree.var_to_vtree()
    f = open(filename, 'r')
    lines = f.readlines()
    all_lines = tqdm(lines) if tqdm else lines
    for line in all_lines:
        node = None
        if line.startswith('c'):
            continue
        elif line.startswith('psdd'):
            node_count = int(line[4:].split(' ')[1])  # ignored
            nodes = {}
        elif line.startswith('F'):  # no FALSE nodes
            pass
        elif line.startswith('T'):
            ...
            node_id, vtree_id, var = [int(x) for x in line[:-2]]
            log_ntheta, log_ptheta = float(line[-2]), float(line[-1])
            if log_ptheta == float('-inf'):
                log_ptheta = -300.0
            if log_ntheta == float('-inf'):
                log_ntheta = -300.0
            vtree = var_to_vtree[var]
            node = PSddNode(SddNode.TRUE, None, vtree, pmanager)
            node.theta = [log_ntheta, log_ptheta]
            node.theta = torch.tensor(node.theta, device=device).float()[:, None, None]
        elif line.startswith('L'):
            node_id, vtree_id, lit = [int(x) for x in line[2:].split()]
            node = pmanager.literals[lit]
            node.theta = [-300.0, -300.0]
            node.theta[node.literal > 0] = 0.0
            node.theta = torch.tensor(node.theta, device=device).float()[:, None, None]
        elif line.startswith('D'):
            line = line[2:].split()
            node_id, vtree_id, size = [int(x) for x in line[:3]]
            line_iter = iter(line[3:])
            elements, theta = list(), list()
            for i in range(size):
                p = nodes[int(next(line_iter))]
                s = nodes[int(next(line_iter))]
                log_theta = float(next(line_iter))
...
def _psdd_wedenigt_repr(self, id_map=None, use_index=False, cluster_idx=0, num_vars_per_cluster=None):
    ...
    if self.is_false():  # no FALSE nodes
        ...
        ntheta = self.theta[0].item()
        ptheta = self.theta[1].item()
        vtree_id = self.vtree.id if id_map is None else id_map[self.vtree.id]
        var = self.vtree.var + cluster_idx * num_vars_per_cluster
        st = 'T %d %d %d %.6f %.6f' % (index(self), vtree_id, var, convert_inf(ntheta), convert_inf(ptheta))
    elif self.is_literal():
        vtree_id = self.vtree.id if id_map is None else id_map[self.vtree.id]
        lit_sign = 1 if self.literal > 0 else -1
        lit = lit_sign * (abs(self.literal) + cluster_idx * num_vars_per_cluster)
        st = 'L %d %d %d' % (index(self), vtree_id, lit)
    else:  # self.is_decomposition()
        vtree_id = self.vtree.id if id_map is None else id_map[self.vtree.id]
        els = self.positive_elements
        st_el = " ".join('%d %d %.6f' % \
                         (index(p), index(s), convert_inf(self.theta[idx].item())) \
                         for idx, (p, s) in enumerate(els))
        st = 'D %d %d %d %s' % (index(self), vtree_id, len(els), st_el)
    return st


def psdd_wedenigt_save(root, filename, id_map=None, cluster_idx=0, num_vars_per_cluster=8):
    """Save a PSDD (Jason format) to file"""
    PSddNode._psdd_repr = _psdd_wedenigt_repr
    _set_index(root)
    with open(filename, 'w') as f:
        f.write(_psdd_jason_file_header)
        f.write('psdd %d %d\n' % (root.vtree.var_count, root._node_count()))
        for i, node in enumerate(root.positive_iter()):
            f.write('%s\n' % node._psdd_repr(use_index=True, id_map=id_map, cluster_idx=cluster_idx,
                                             num_vars_per_cluster=num_vars_per_cluster))
