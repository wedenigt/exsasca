...
class SddNode:
    ...
    def positive_iter(self, first_call=True, clear_data=False):
        """post-order (children before parents) generator of positive circuits"""
        if self._bit: return
        self._bit = True

        if self.is_decomposition():
            for p, s in self.elements:
                if p.is_false_sdd or s.is_false_sdd:
                    continue
...
class NormalizedSddNode(SddNode):
    ...
    def rand_parameters(self):
        ...
        for node in self.positive_iter():
            ...
            node.theta = torch.nn.functional.softmax(node.theta,
                                                     dim=-1)  # dict( (el,0.0) for el in root.positive_elements )
            # node.theta_sum = 0.0
    ...
    def get_mpe(self, batch_size, clear_data=True):
        ...
        for node in self.as_positive_list(clear_data=clear_data):
            ...
            if node.is_false():
                ...
                data = torch.where(node.theta.argmax(dim=-2) > 0,
                                   torch.tensor(node.vtree.var, device=DEVICE, dtype=tint),
                                   torch.tensor(-node.vtree.var, device=DEVICE, dtype=tint))
                data = data.unsqueeze(dim=-2)
                # assert(len(data.shape) == 3 and data.shape[0] == len(node.theta) and data.shape[1] == 1)

            elif node.is_literal():
                data = torch.tensor([node.literal], device=DEVICE, dtype=tint).unsqueeze(0).unsqueeze(-1).expand(
                    batch_size, 1, self.num_reps)
                ...
                if len(node.positive_elements) == 1:
                    max_branch = torch.tensor([0], device=DEVICE, dtype=tint).unsqueeze(-1).unsqueeze(-1).expand(1,
                                                                                                                 batch_size,
                                                                                                                 self.num_reps)
    ...
    def ff_ll(self, litleaves, target, clear_data=True):
        ...
        for node in self.as_positive_list(clear_data=clear_data):
            ...
            if node.is_false():
                ...
                # idx = (target[:, node.vtree.var - 1] > 0)[0].int()
                # data = litleaves[node.vtree.var - 1][idx]

            elif node.is_literal():
                data = (target[:, abs(node.literal) - 1] == (node.literal > 0))  # .log()#.float()
                data = torch.where(data.unsqueeze(-1).expand(-1, K), litleaves[abs(node.literal) - 1][node.literal > 1],
    ...
    def ll(self, target, log_space=True, clear_data=True):
        ...
        for node in self.as_positive_list(clear_data=clear_data):
            ...
            if node.is_false():
                ...
                idx = target[:, node.vtree.var - 1] > 0
                data = torch.gather(node.theta, dim=1, index=idx.view(-1, 1, 1).long()).reshape(-1, 1)
                # data = torch.where((target[:, node.vtree.var - 1] > 0), node.theta[:, 1], node.theta[:, 0])
                ...
                data = torch.where(data.unsqueeze(-1).expand(-1, K), torch.tensor(0., device=DEVICE),
                                   torch.tensor(-300., device=DEVICE))
